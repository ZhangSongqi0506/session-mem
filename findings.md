...
- **反思**：曾一度取消候选池的 `seen` 前置过滤，意图"保证补满 3 个"。但实体扩展的本质是**补充**召回池缺失的相关 cell，而非凑数。如果 BM25 排名前 3 的 cell 都已经在召回池中，说明扩展查询没有带来新的高价值候选，此时继续向下翻找只会拉入低相关 cell，违背补充机制的设计初衷。
- **结论**：回退到 9.2 原实现——候选池先过滤 `seen`，再取 top `extra_limit`，只做真正的补充。
