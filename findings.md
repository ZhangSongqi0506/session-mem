...
- **Status:** complete（代码已修改、测试通过）
- **逻辑**：
  1. **全部 session cell** 参与 BM25 评分，不预先过滤 `seen`，保证打分公平。
  2. 取 **全局 BM25 top 3**。
  3. 只把 top 3 中**不在 `seen` 里**的 cell 补充进激活列表，数量可能为 0~3 个。
- **设计 rationale**：
  - 若前置过滤 `seen`，已被双路召回的高 BM25 分 cell 会占据 top 3 名额，导致真正未被召回的新候选被挤出视野。
  - 但如果 top 3 都已在召回池中，说明扩展查询没有带来新的有效候选，此时自然补充 0 个，不强行凑数。
