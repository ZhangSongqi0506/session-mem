...
- **Status:** complete（代码已修改、测试通过）
- **问题**：9.2 初版实现中，BM25 实体扩展的候选池预先排除了 `seen`（已入选 cell）。这导致如果本应在 BM25 排名前 3 的 cell 已被双路召回选中，实际通过实体扩展补充的新 cell 数量会缩水，甚至为 0。
- **修复**：
  1. 取消候选池的 `seen` 前置过滤，让**全部 session cell** 参与 BM25 评分，获得全局排名。
  2. 排序后遍历，遇到 `seen` 中的 cell 直接跳过，继续向下寻找，直到取满 `extra_limit=3` 个真正新增的 cell 为止。
- **效果**：实体扩展的 "3 个补充名额" 得到保障，不会因为高 BM25 分 cell 已被召回而浪费扩展机会；同时已激活 cell 不会重复加入。
- **benchmark 策略**：并入 9.2，全部完成后统一跑 **v7 benchmark**。
...
- **修复方案（已讨论确定）**：
  1. **方案 A（频率阈值过滤）**：计算各实体覆盖的 cell 比例，超过 **50%** 的高频实体不再触发 `find_by_entity` 共现扩展。
  2. **方案 B（BM25 替代实体共现）**：
     - 将当前激活 cell 的 entities 拼接成扩展查询（如 `"Jon dance studio Marley flooring"`）。
     - 对未入选的候选 cell 直接调用 `hybrid_search.keyword_scores()` 做 BM25 评分。
     - 高频实体（如 `Jon`）因 IDF 极低而贡献微弱；低频特异性实体（如 `Marley flooring`）因 IDF 高而显著提权。
     - 取 BM25 得分最高的前 `extra_limit` 个 cell 加入激活列表。
  3. **优先采用方案 B**：因为 BM25 的 IDF 机制天然解决了高频实体的噪音问题，无需硬编码 50% 阈值，且与现有检索代码更统一。
- **预期收益**：
