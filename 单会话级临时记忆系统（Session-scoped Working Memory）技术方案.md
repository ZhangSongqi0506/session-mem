...
当 Buffer 达到检测阈值时，调用轻量 LLM 对当前累积的全部轮次进行**多边界语义分析**，LLM 返回零个、一个或多个**切分点索引**，标明 Buffer 内存在几处主题转折。
...
| **检测结果** | **处理策略** | **结果状态** |
| --- | --- | --- |
| **无边界**（全程语义连贯） | 继续累积，保留原文在 Buffer 内，等待下一检测点（512→1024→...） | 累积态 |
| **单一边界** | 按切分索引将 Buffer 拆分为两段：<br>1. **旧主题**：前段打包生成 Cell<br>2. **新主题**：后段（触发转折的轮次及后续）保留在 Buffer 中，作为新逻辑单元的种子 | 切分态 |
| **多个边界** | 按切分索引将 Buffer 拆分为 **N+1** 段，前 N 段分别打包生成 **N 个 Cell**，最后一段保留在 Buffer 中继续累积 | 切分态 |

**优势**：避免传统固定窗口（如每 5 轮强制切分）可能造成的**逻辑链断裂**（如切断"因为...所以..."的因果关系），确保每个 Cell 承载**语义闭合的完整话题单元**；同时支持一次检测产出多个 Cell，解决长累积窗口内存在多个完整子话题时的切分滞后问题。
...
*   **滑动保留**：触发切分后，**最后一个切分点之后的轮次不进入任何旧 Cell**，而是留在 Buffer 作为下一个逻辑单元的种子。若存在多个切分点，前 N 段依次生成 N 个 Cell，最后一段始终保留。这保证了相邻 Cell 间的**时序连续性**（避免丢轮），同时支持一次检测产出多个 Cell。
...
*   **语义边界**（充分条件，满足其一即可）：
    ...
    *   上下文关联度骤降（当前轮与某历史轮次 Embedding 相似度 <0.6）
        

**边界检测实现**：

*   主路径：调用模型分析当前 Buffer 内全部轮次，返回**切分点索引列表**（如 `[3, 6]` 表示第 3 轮后和第 6 轮后各有一个边界）。系统按切分点将 Buffer 切分为 N+1 段，前 N 段分别生成 Cell，最后一段保留在 Buffer 中继续累积。
    
*   Fallback：规则匹配（检测特定标点、关键词）+ 向量相似度阈值。Fallback 时最多返回单个切分点。
